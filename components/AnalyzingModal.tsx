...
const STEPS = [
  { icon: Globe,     label: "Fetching store page",          duration: 3000 },
  { icon: Shield,    label: "Checking security signals",    duration: 2500 },
  { icon: FileText,  label: "Scanning policy pages",        duration: 3000 },
  { icon: Mail,      label: "Verifying contact info",       duration: 2000 },
  { icon: Brain,     label: "Running AI analysis",          duration: 5000 },
  { icon: Sparkles,  label: "Generating your report",       duration: 2000 },
];
...
export function AnalyzingModal({ url, open }: Props) {
  ...
  // Auto-advance steps based on durations
  useEffect(() => {
    if (!open) {
      setCurrentStep(0);
      setCompletedSteps([]);
    ...
    STEPS.forEach((step, i) => {
      // Mark step as "in progress" at start of its window
      timers.push(setTimeout(() => setCurrentStep(i), elapsed));

      // Mark step as "completed" near end of its window
          ...
          >
            ...
            <div className="p-8">
              ...
              <div className="mb-7 flex items-start gap-4">
                ...
                <div>
                  <h3 className="text-lg font-bold">Analyzing store…</h3>
                    ...
                    >
                      ...
                      {isActive && (
                        <div className="ml-auto flex gap-1">
                          {[0, 1, 2].map((d) => (
                            <motion.span key={d} className="h-1.5 w-1.5 rounded-full"
